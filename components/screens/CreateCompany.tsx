...
const LEGAL_FORMS_FR = ['SARL', 'SA', 'SAS', 'SASU', 'SNC', 'GIE', 'Entreprise individuelle', 'Autre'];
const LEGAL_FORMS_EN = ['LLC', 'Corporation', 'Partnership', 'Sole trader', 'Other'];
const CURRENCIES = ['GNF','EUR', 'USD', 'MAD', 'DZD'];
...
const COUNTRIES_FR = ["Côte d'Ivoire", 'Sénégal', 'Mali', 'Burkina Faso', 'Niger', 'Guinée', 'Togo', 'Bénin', 'Cameroun', 'Congo', 'Gabon', 'RDC', 'Autre'];
...
export function CreateCompany() {
  ...
  const createCompany = async () => {
    setError('');
    if (!companyName.trim()) {
      setError(fr ? 'Le nom de la societe est obligatoire.' : 'Company name is required.');
      setStep('identity');
      return;
    }
    ...
    if (!res.ok) {
      setError(fr ? 'Impossible de creer la societe.' : 'Could not create company.');
      return;
    }
  ...
  const handleNext = () => {
    ...
    else void createCompany();
  };

  return (
    <div>
      <PageHeader
        title={fr ? 'Nouvelle société' : 'New company'}
        subtitle={fr ? 'Renseignez les informations de la société pour créer le dossier.' : 'Fill in the company details to create the file.'}
        actions={
          <>
            <Btn variant="ghost" onClick={() => router.push('/companies')}>{fr ? 'Annuler' : 'Cancel'}</Btn>
            <Btn variant="primary" icon={stepIndex === STEPS.length - 1 ? <Icons.check /> : <Icons.arrowRight />} onClick={handleNext} disabled={saving}>
              {stepIndex === STEPS.length - 1 ? (fr ? 'Créer la société' : 'Create company') : (fr ? 'Suivant' : 'Next')}
            </Btn>
          </>
        }
      />
      ...
      <div className="px-8 py-6 pb-12 grid gap-6" style={{ gridTemplateColumns: '1fr 280px', alignItems: 'start' }}>
              ...
              <div className="px-5 py-5 grid gap-4">
                <div className="grid grid-cols-2 gap-4">
                  <Field label={fr ? 'Nom de la société *' : 'Company name *'}>
                    <Input placeholder={fr ? 'Nom de la societe' : 'Company name'} value={companyName} onChange={(e) => setCompanyName(e.target.value)} />
                  </Field>
                  <Field label={fr ? 'Raison sociale' : 'Legal name'}>
                    <Input placeholder={fr ? 'Raison sociale complète' : 'Full legal name'} />
                  </Field>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <Field label={fr ? 'Nom commercial' : 'Commercial name'}>
                    <Input placeholder={fr ? 'Nom commercial si différent' : 'Trade name if different'} />
                  </Field>
                  <Field label={fr ? 'Forme juridique *' : 'Legal form *'}>
                    <Select>
                      <option value="">{fr ? '— Sélectionner —' : '— Select —'}</option>
                      {(fr ? LEGAL_FORMS_FR : LEGAL_FORMS_EN).map((f) => <option key={f}>{f}</option>)}
                    </Select>
                  </Field>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <Field label={fr ? 'Numéro RCCM *' : 'RCCM number *'}>
                    <Input placeholder="CI-ABJ-2024-B-XXXXX" />
                  </Field>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field label={fr ? "Secteur d'activité *" : 'Business sector *'}>
                    <Input placeholder={fr ? 'Ex: Industrie agroalimentaire' : 'e.g., Food & Beverage'} />
                  </Field>
                  <Field label={fr ? 'Activité principale' : 'Main activity'}>
                    <Input placeholder={fr ? 'Description courte' : 'Short description'} />
                  </Field>
                </div>
            ...
            <Card>
              ...
              <div className="px-5 py-5 grid gap-4">
                <Field label={fr ? 'Adresse *' : 'Address *'}>
                  <Input placeholder={fr ? 'Rue, avenue, lot…' : 'Street, avenue, lot…'} />
                </Field>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <Field label={fr ? 'Pays *' : 'Country *'}>
                    <Select>
                      {COUNTRIES_FR.map((c) => <option key={c}>{c}</option>)}
                    </Select>
                  </Field>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <Field label="Email">
                    <Input type="email" placeholder="contact@société.ci" />
                  </Field>
                </div>
                <Field label={fr ? 'Site web' : 'Website'}>
                  <Input placeholder="https://www.société.ci" />
                </Field>
              </div>
            </Card>
          )}
              ...
              <div className="px-5 py-5 grid gap-4">
                <div className="grid grid-cols-2 gap-4">
                  <Field label={fr ? 'Centre des impôts' : 'Tax center'}>
                    <Input placeholder={fr ? 'Ex: DGE, DRFPl-Abidjan…' : 'e.g., Large Taxpayers, District…'} />
                  </Field>
                ...
                <div className="p-4 bg-bg rounded-xl border border-line">
                  <div className="text-[12.5px] text-muted">
                    {fr
                      ? 'Le référentiel SYSCOHADA Révisé est utilisé par défaut pour les états OHADA. Modifiez uniquement si votre entité utilise un autre référentiel.'
                      : 'SYSCOHADA Revised is used by default for OHADA statements. Change only if your entity uses a different standard.'}
                  </div>
                </div>
              </div>
            </Card>
          ...
          {step === 'people' && (
            <Card>
              ...
              <div className="px-5 py-5 grid gap-5">
                <div>
                  <div className="text-[11px] font-bold text-muted uppercase tracking-[.08em] mb-3">
                    {fr ? 'Dirigeant' : 'Manager'}
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <Field label={fr ? 'Nom du dirigeant' : 'Manager name'}>
                      <Input placeholder={fr ? 'Nom complet' : 'Full name'} />
                    </Field>
                    <Field label={fr ? 'Fonction' : 'Title'}>
                      <Input placeholder={fr ? 'Ex: Directeur Général' : 'e.g., CEO'} />
                    </Field>
                  </div>
                </div>
                <div>
                  <div className="text-[11px] font-bold text-muted uppercase tracking-[.08em] mb-3">
                    {fr ? 'Comptable' : 'Accountant'}
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <Field label={fr ? 'Nom du comptable' : 'Accountant name'}>
                      <Input placeholder={fr ? 'Nom complet' : 'Full name'} />
                    </Field>
                    <Field label={fr ? 'Contact' : 'Contact'}>
                      <Input placeholder={fr ? 'Email ou téléphone' : 'Email or phone'} />
                    </Field>
                  </div>
                </div>
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Field label={fr ? "Nom de l'auditeur" : 'Auditor name'}>
                      <Input placeholder={fr ? 'Nom complet ou cabinet' : 'Full name or firm'} />
                    </Field>
                    <Field label={fr ? 'Contact' : 'Contact'}>
                      <Input placeholder={fr ? 'Email ou téléphone' : 'Email or phone'} />
                    </Field>
                ...
                <Field label={fr ? 'Notes internes' : 'Internal notes'}>
                  <textarea rows={3} className="w-full px-3 py-2.5 border border-line rounded-lg text-[13px] text-ink bg-bg focus:outline-none focus:border-rust transition-colors resize-none"
        ...
        <Card className="p-4 sticky top-[72px]">
          ...
          <div className="mt-4 p-3 bg-bg rounded-lg border border-line-2 text-[12px] text-muted">
